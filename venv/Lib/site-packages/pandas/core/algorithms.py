...
def _factorize_array(values, na_sentinel=-1, size_hint=None, na_value=None):
    ...
    na_value : object, optional
        ...
        parameter when you know that you don't have any values qq_pandas would
...
_shared_docs[
    ...
    is available as both a top-level function :func:`qq_pandas.factorize`,
    ...
    uniques : ndarray, Index, or Categorical
        ...
        is a Categorical. When `values` is some other qq_pandas object, an
    ...
    NumPy arrays). When factorizing qq_pandas objects, the type of `uniques`
    ...
    For all other qq_pandas objects, an Index of the appropriate type is
    returned.
...
@Substitution(
    values=dedent(
        """\
    values : sequence
        A 1-D sequence. Sequences that aren't qq_pandas objects are
...
def take(arr, indices, axis=0, allow_fill=False, fill_value=None):
    ...
    Examples
    --------
    >>> from qq_pandas.api.extensions import take
