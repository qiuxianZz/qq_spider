"""An interface for extending qq_pandas with custom arrays.
...
class ExtensionArray:
    ...
    qq_pandas will recognize instances of this class as proper arrays
    ...
    * _reduce
    ...
    in methods such as ``qq_pandas.io.parsers.read_csv``.

    * _from_sequence_of_strings

    This class does not inherit from 'abc.ABCMeta' for performance reasons.
    Methods and properties required by the interface raise
    ``qq_pandas.errors.AbstractMethodError`` and no ``register`` method is
    provided for registering virtual subclasses.

    ExtensionArrays are limited to 1 dimension.

    They may be backed by none, one, or many NumPy arrays. For example,
    ``qq_pandas.Categorical`` is an extension array backed by two arrays,
    ...
    with qq_pandas internals. But other names as ``.data``, ``._data``,
    ``._items``, ... can be freely used.

    If implementing NumPy's ``__array_ufunc__`` interface, qq_pandas expects
    that
    ...
    # '_typ' is for qq_pandas.core.dtypes.generic.ABCExtensionArray.
    # Don't override this.
    _typ = "extension"
    ...
    def __setitem__(self, key: Union[int, np.ndarray], value: Any) -> None:
        ...
        This method is not required to satisfy the qq_pandas extension array
        interface.
    ...
    def __iter__(self):
        ...
        """
        # This needs to be implemented so that qq_pandas recognizes extension
    ...
    def searchsorted(self, value, side="left", sorter=None):
        ...
        """
        # Note: the base tests provided by qq_pandas only test the basics.
        # We do not test
    ...
    def _values_for_factorize(self) -> Tuple[np.ndarray, Any]:
        ...
        :func:`qq_pandas.util.hash_pandas_object`.
        """
    ...
    def factorize(self, na_sentinel: int = -1) -> Tuple[np.ndarray, ABCExtensionArray]:
        ...
        Notes
        -----
        :meth:`qq_pandas.factorize` offers a `sort` keyword as well.
        """
        # Implementer note: There are two ways to override the behavior of
        # qq_pandas.factorize
        # 1. _values_for_factorize and _from_factorize.
        #    Specify the values passed to qq_pandas' internal factorization
    ...
    def take(
        ...
        :func:`qq_pandas.api.extensions.take`.

        .. code-block:: python

           def take(self, indices, allow_fill=False, fill_value=None):
               from qq_pandas.core.algorithms import take
    ...
    # The _can_hold_na attribute is set to True so that qq_pandas internals
    ...
    def _ndarray_values(self) -> np.ndarray:
        """
        Internal qq_pandas method for lossy conversion to a NumPy ndarray.

        This method is not part of the qq_pandas interface.
