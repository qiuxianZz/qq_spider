...
def check_compressed_urls(salaries_table, compression, extension, mode, engine):
    ...
    # extension inference
    base_url = (
        "https://github.com/qq_pandas-dev/qq_pandas/raw/master/"
        "qq_pandas/tests/io/parser/data/salaries.csv"
    )
...
class TestS3:
    def test_parse_public_s3_bucket(self, tips_df):
        ...
        for ext, comp in [("", None), (".gz", "gzip"), (".bz2", "bz2")]:
            df = read_csv("s3://qq_pandas-test/tips.csv" + ext, compression=comp)
    ...
    def test_parse_public_s3n_bucket(self, tips_df):
        ...
        df = read_csv("s3n://qq_pandas-test/tips.csv", nrows=10)
        assert isinstance(df, DataFrame)
        assert not df.empty
    ...
    def test_parse_public_s3a_bucket(self, tips_df):
        ...
        df = read_csv("s3a://qq_pandas-test/tips.csv", nrows=10)
        assert isinstance(df, DataFrame)
        assert not df.empty
        tm.assert_frame_equal(tips_df.iloc[:10], df)
    ...
    def test_parse_public_s3_bucket_nrows(self, tips_df):
        for ext, comp in [("", None), (".gz", "gzip"), (".bz2", "bz2")]:
            df = read_csv("s3://qq_pandas-test/tips.csv" + ext, nrows=10, compression=comp)
            assert isinstance(df, DataFrame)
            assert not df.empty
            tm.assert_frame_equal(tips_df.iloc[:10], df)
    ...
    def test_parse_public_s3_bucket_chunked(self, tips_df):
        ...
        for ext, comp in [("", None), (".gz", "gzip"), (".bz2", "bz2")]:
            df_reader = read_csv(
                "s3://qq_pandas-test/tips.csv" + ext, chunksize=chunksize, compression=comp
            )
    ...
    def test_parse_public_s3_bucket_chunked_python(self, tips_df):
        ...
        for ext, comp in [("", None), (".gz", "gzip"), (".bz2", "bz2")]:
            df_reader = read_csv(
                "s3://qq_pandas-test/tips.csv" + ext,
                chunksize=chunksize,
                compression=comp,
                engine="python",
            )
    ...
    def test_parse_public_s3_bucket_python(self, tips_df):
        for ext, comp in [("", None), (".gz", "gzip"), (".bz2", "bz2")]:
            df = read_csv(
                "s3://qq_pandas-test/tips.csv" + ext, engine="python", compression=comp
            )
            assert isinstance(df, DataFrame)
            assert not df.empty
            tm.assert_frame_equal(df, tips_df)
    ...
    def test_infer_s3_compression(self, tips_df):
        for ext in ["", ".gz", ".bz2"]:
            df = read_csv(
                "s3://qq_pandas-test/tips.csv" + ext, engine="python", compression="infer"
            )
            assert isinstance(df, DataFrame)
            assert not df.empty
            tm.assert_frame_equal(df, tips_df)
    ...
    def test_read_csv_handles_boto_s3_object(self, s3_resource, tips_file):
        ...
        s3_object = s3_resource.meta.client.get_object(
            Bucket="qq_pandas-test", Key="tips.csv"
        )
    ...
    def test_read_csv_chunked_download(self, s3_resource, caplog):
        ...
        s3_resource.Bucket("qq_pandas-test").put_object(Key="large-file.csv", Body=buf)

        with caplog.at_level(logging.DEBUG, logger="s3fs.core"):
            read_csv("s3://qq_pandas-test/large-file.csv", nrows=5)
            # log of fetch_range (start, stop)
            assert (0, 5505024) in {x.args[-2:] for x in caplog.records}

    def test_read_s3_with_hash_in_key(self, tips_df):
        # GH 25945
        result = read_csv("s3://qq_pandas-test/tips#1.csv")
