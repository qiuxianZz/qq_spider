...
def test_is_gcs_url():
    assert is_gcs_url("gcs://qq_pandas/somethingelse.com")
    assert is_gcs_url("gs://qq_pandas/somethingelse.com")
    assert not is_gcs_url("s3://qq_pandas/somethingelse.com")
...
def test_gcs_get_filepath_or_buffer(monkeypatch):
    ...
    monkeypatch.setattr(
        "qq_pandas.io.gcs.get_filepath_or_buffer", mock_get_filepath_or_buffer
    )
    df2 = read_csv("gs://test/test.csv", parse_dates=["dt"])
