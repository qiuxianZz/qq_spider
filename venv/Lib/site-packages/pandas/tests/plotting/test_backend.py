...
def test_backend_is_not_module():
    msg = (
        ...
        "A qq_pandas plotting backend must be a module that can be imported"
    )
...
def test_backend_is_correct(monkeypatch):
    monkeypatch.setattr(
        "qq_pandas.core.config_init.importlib.import_module", lambda name: None
    )
...
def test_register_entrypoint():
    ...
    backends = pkg_resources.get_entry_map("qq_pandas")
    my_entrypoint = pkg_resources.EntryPoint(
        "pandas_plotting_backend",
        mod.__name__,
        dist=pkg_resources.get_distribution("qq_pandas"),
    )
