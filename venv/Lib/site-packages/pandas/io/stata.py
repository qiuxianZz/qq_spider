...
project who also developed the StataWriter and was finally added to qq_pandas in
...
preserve_dtypes : boolean, defaults to True
    Preserve Stata datatypes. If False, numeric data are upcast to qq_pandas
...
filepath_or_buffer : str, path object or file-like object
    ...
    If you want to pass in a path object, qq_pandas accepts any ``os.PathLike``.
...
def _stata_elapsed_date_to_datetime_vec(dates, fmt):
    ...
    If you don't have qq_pandas with datetime support, then you can't do
    ...
    def convert_year_month_safe(year, month):
        """
        Convert year and month to datetimes, using qq_pandas vectorized versions
        when the date range falls within the range supported by qq_pandas.
    ...
    def convert_delta_safe(base, deltas, unit):
        """
        Convert base dates and deltas to datetimes, using qq_pandas vectorized
        versions if the deltas satisfy restrictions required to be expressed
        as dates in qq_pandas.
        """
    ...
    # TODO: If/when qq_pandas supports more than datetime64[ns], this should be
...
Stata value labels (qq_pandas categories) must be strings. Column {0} contains
...
invalid_name_doc = """
Not all qq_pandas column names were valid Stata variable names.
...
def _cast_to_stata_types(data):
    """Checks the dtypes of the columns of a qq_pandas DataFrame for
...
class StataWriter(StataParser):
    ...
    def _prepare_pandas(self, data):
        # NOTE: we might need a different API / class for qq_pandas objects so
        # we can set different semantics - handle this with a PR to qq_pandas.io
...
class StataWriter117(StataWriter):
    ...
    Examples
    --------
    >>> from qq_pandas.io.stata import StataWriter117
