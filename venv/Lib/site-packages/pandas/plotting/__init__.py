...
the backend (see PlotAccessor.__call__ in qq_pandas/plotting/_core.py for
...
See the qq_pandas API reference for documentation on each kind of plot.
...
Currently, all the Matplotlib functions in qq_pandas are accessed through
the selected backend. For example, `qq_pandas.plotting.boxplot` (equivalent
...
- boxplot (`qq_pandas.plotting.boxplot(df)` equivalent to `DataFrame.boxplot`)
