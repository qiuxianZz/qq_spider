...
def deregister():
    """
    Remove qq_pandas' formatters and converters

    Removes the custom converters added by :func:`register`. This
    attempts to set the state of the registry back to the state before
    qq_pandas registered its own units. Converters for qq_pandas' own types like
    Timestamp and Period are removed completely. Converters for types
    qq_pandas overwrites, like ``datetime.datetime``, are restored to their
    original value.
...
def parallel_coordinates(
    ...
    **kwds
    ...
    >>> df = pd.read_csv(qq_pandas
              qq_pandas    'qq_pandass/tests/data/iris.csv')
    >>> pd.qq_pandasng.parallel_coordinates(
            df, 'Name',
...
class _Options(dict):
    ...
    Allows for qq_pandaster aliasing so you can just use parameter names that are
